...
.. admonition:: Pending v0.3 re-run — the numbers below are superseded
   ...
   *Status 2026-08-19.*  ``sensitivity_fisher/fisher_study.npz`` is
   campaign-fresh (2026-07-16), but its ``param_names`` no longer match
   :data:`hod_mod.forecast.forward_jax.PARAM_NAMES`: the pressure sector was
   re-parametrised afterwards (``p0_pressure``, ``c500_pressure``,
   ``gamma_pressure``, ``alpha_pressure``, ``beta_out_pressure`` →
   ``p03_pressure``, ``c_dpm_pressure``, ``alpha_in_pressure``,
   ``alpha_tr_pressure``, ``alpha_out_pressure``).  ``make_sensitivity_figures``
   therefore recomputes the 229×111 Jacobian on whatever code is checked out
   instead of reusing the campaign product, so the figures here are **not**
   regenerated by ``collect_and_plot.sh`` (set ``REFRESH_SENSITIVITY=1`` to
   override).  The page also still describes the earlier 31-parameter study; the
   archived version is under ``sensitivity_fisher/archive_31param_2026-07-02/``.
   Re-run ``run_sensitivity_study`` to bring the two back into step.  See
   :doc:`oarsub_campaign`.
