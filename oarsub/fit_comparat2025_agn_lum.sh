#!/usr/bin/env bash
# =============================================================================
# GRICAD / OAR job: Comparat+2025 fixed-ZM15 X-ray fit — S1, MAP, agn-lum.
#
...
#
# agn-lum frees the HamAGNModel luminosity overrides (scatter_lx, log10_A_kcorr,
# log10_A_dc) on top of the 4 'all' params; requires --agn-model ham.  No gas/AGN
# profile rebuilds, so this is the fast preset (~minutes).  NOTE: in the w_θ-only
# fit the 3 luminosity parameters are degenerate with log10_A_AGN (flat
# directions) — see oarsub/README.md.
#
# Submit:  oarsub --project your-oar-project -S ./oarsub/fit_comparat2025_agn_lum.sh
...
#OAR --stderr oarsub/logs/%jobid%.c2025_agn_lum.err
...
OUT_DIR="${HOD_MOD_RESULTS}/fits/comparat2025_fixedZM15_agn-lum${ECF_SUFFIX}_${VTAG}"
...
python -m hod_mod.scripts.fitting.fit_comparat2025 \
    ...
    --free-params agn-lum --agn-model ham ${ECF_FLAG} \
    --out-dir "${OUT_DIR}"
