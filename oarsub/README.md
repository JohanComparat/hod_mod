...
```bash
./oarsub/submit_campaign.sh pr-orphans benchmarks_map --devel   # 1-line smoke
./oarsub/submit_campaign.sh pr-orphans benchmarks_map
./oarsub/submit_campaign.sh pr-orphans all                      # everything
```

or by hand:

```bash
oarsub --project pr-orphans -l /nodes=1/core=8,walltime=02:00:00 \
...
Submit with `oarsub --project pr-orphans -S ./oarsub/fit_comparat2025_<preset>.sh`
