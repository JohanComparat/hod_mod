...
def _pk_to_wp(rp_arr: np.ndarray, log_k: np.ndarray, log_pk: np.ndarray) -> np.ndarray:
    ...
    :func:`hod_mod.observables.clustering._delta_sigma_from_pgm`:
...
class HaloModelCrossSpectra:
    ...
    Wraps a :class:`~hod_mod.observables.clustering.FullHaloModelPrediction`
    ...
    def __init__(
        ...
        ecf_gas_table : callable | None
            Optional true eROSITA gas energy-conversion factor as a **per-halo
            weight** ``ECF_gas(m200 [Msun/h])`` [cts/s per erg/s/cm²] (TM0 survey
            ARF+RMF evaluated at the halo temperature kT(M); see
            :class:`hod_mod.gas.ErositaResponse` and ``_Infrastructure.
            enable_ecf``).  When set, the gas emissivity ``X_uk`` is multiplied by
            this weight so the gas leg predicts observed counts rather than
            intrinsic energy.  Works for both the density-only and full-APEC paths.
        ecf_agn : float | None
            Optional true ECF for the AGN (absorbed power law Γ=1.9) [same units].
            When set, the AGN luminosity conversion carries this factor, putting
            gas and AGN on a common observed-count footing.
        """
        self._fhmp       = fhmp
        ...
        self._agn        = agn_model
        self._ecf_gas_table = ecf_gas_table
        self._ecf_agn       = ecf_agn
    ...
    def _pk_tables_gy(
        ...
        where ỹ(k|M,z) is from :class:`~hod_mod.gas.PressureProfileA10`
        and has units (Mpc/h)².
    ...
    def _pk_tables_gX(
        ...
        if x_uk_override is not None:
            # Emulator fast path: a precomputed raw emissivity FT X̃(k|M),
            # already divided by Λ_ref and pre-scaled to the requested n_e,0.3²
            # by the caller.  Skips the ~1.3 s emissivity_full_uk FT; the cheap
            # ECF weight and β_gas / β_pressure tilts below still apply.
            X_uk = np.asarray(x_uk_override, dtype=float)
        elif _has_full:
        ...
        # Fold the true eROSITA gas ECF as a per-halo weight ECF_gas(kT(M)) so the
        # gas emissivity becomes observed counts (works for both the density-only
        # and full-APEC paths; "tabulated band averages").  ECF_gas(M) is supplied
        # as a callable over m200 [Msun/h] by ``ErositaResponse`` + a kT-M relation.
        if self._ecf_gas_table is not None:
            w_ecf = np.asarray(self._ecf_gas_table(np.asarray(m_np, dtype=float)),
                               dtype=float)
            X_uk = np.asarray(X_uk) * w_ecf[None, :]
        ...
        if self._agn is not None:
            ...
            if self._agn_has_hod:
                ...
                if getattr(self._agn, "_pair_with_galaxy_occupation", False):
                    # Lau+2025 (A7/A8) modified for the galaxy×AGN-emission
                    # cross: the 1-halo term pairs the GALAXY occupation
                    # profile with the AGN occupation profile (not the AGN
                    # occupation alone).  In a halo of mass M this counts
                    # galaxy×AGN pairs:  (N_c^g + N_s^g u)(N_c^A + N_s^A u).
                    # The 2-halo cross keeps galaxy b_eff × AGN-occupation
                    # bias (computed below via agn_occ_2h and b_eff).
                    gal_occ_1h = nc_j[None, :] + ns_j[None, :] * uk_j
                    agn_occ_1h = gal_occ_1h * agn_occ_1h
            else:
            ...
            # carries its own count-conversion, matching the ECF-weighted gas.
            if self._ecf_agn is not None:
                agn_conv = agn_conv * float(self._ecf_agn)
    ...
    def _pk_tables_XX(
        ...
        return {
            "log_k":            log_k,
            "log_pXX":          _safe_log(P_XX,              1e-30),
            "log_pXX_gas_gas":  _safe_log(P_XX_1h_gas_gas + P_XX_2h_gas_gas, 1e-30),
            "log_pXX_cross":    _safe_log(P_XX_1h_cross   + P_XX_2h_cross,   1e-30),
            "log_pXX_agn_agn":  _safe_log(P_XX_1h_agn_agn + P_XX_2h_agn_agn, 1e-30),
            "log_pXX_2h":       _safe_log(P_XX_2h,          1e-30),
            "log_pXX_1h":       _safe_log(P_XX_1h,          1e-30),
        }
    ...
    def emissivity_xuk_per_z(self, z_arr, theta_cosmo, hod_params):
        ...
        :meth:`angular_cl_gX` consumes via ``x_uk_override``.  This isolates the
        expensive full-APEC FT (``emissivity_full_uk``, ~1.3 s/z) so a joint fit
        can cache it on a (p2, r_max) grid and skip it at evaluation time.  The
        cached value is at this instance's ``self._dp._ne_03``; the caller scales
        by ``(n_e,0.3 / n_e,0.3_ref)²`` before passing it back.

        Requires the full-APEC profiles (``_dp``/``_pp``/``_mp``/``_cooling_fn``).
        """
        if self._dp is None or self._pp is None or self._mp is None or self._cooling_fn is None:
            raise ValueError("emissivity_xuk_per_z needs the full-APEC profiles set "
                             "(_dp, _pp, _mp, _cooling_fn).")
        lambda_ref = float(self._cooling_fn(np.array([1.0]), np.array([0.3]))[0])
        out = []
        for zi in np.asarray(z_arr, dtype=float):
            sc = self._get_static_cache(float(zi), theta_cosmo, hod_params)
            X = self._dp.emissivity_full_uk(
                k_arr               = sc["k_np"],
                m200_arr            = sc["m_np"],
                r200_arr            = sc["r_delta"],
                z                   = float(zi),
            ...
            out.append(np.asarray(X, dtype=float) / lambda_ref)
        return out

    def emissivity_xuk_bands_per_z(self, z_arr, theta_cosmo, hod_params, cooling_fns):
        """Multi-band version of :meth:`emissivity_xuk_per_z` for the energy bands.

        Computes the per-z, per-band raw emissivity FT ``X̃_b(k|M)/Λ_ref,b`` in one
        batched FT per z (``GasDensityDPM.emissivity_full_uk_bands``).  Each band is
        divided by its OWN reference cooling ``Λ_ref,b = cooling_fns[b](1 keV,
        0.3 Z⊙)`` so band ``b`` feeds ``angular_cl_gX(x_uk_override=...)`` exactly
        like the broad-band path (which divides by the broad-band Λ_ref).

        Returns a list over z; each entry is a list over bands of (Nk, NM) arrays
        (so ``out[iz][b]`` is a valid ``x_uk_override`` for band ``b`` at z-index
        ``iz``).  ``self._dp``/``_pp``/``_mp`` must be set (the cooling tables are
        passed explicitly, not taken from ``self._cooling_fn``).
        """
        if self._dp is None or self._pp is None or self._mp is None:
            raise ValueError("emissivity_xuk_bands_per_z needs _dp, _pp, _mp set.")
        lam_ref = np.array([float(cf(np.array([1.0]), np.array([0.3]))[0])
                            for cf in cooling_fns])              # (Nb,)
        out = []
        for zi in np.asarray(z_arr, dtype=float):
            sc = self._get_static_cache(float(zi), theta_cosmo, hod_params)
            Xb = self._dp.emissivity_full_uk_bands(
                k_arr               = sc["k_np"],
                m200_arr            = sc["m_np"],
                r200_arr            = sc["r_delta"],
                z                   = float(zi),
                ...
                pressure_profile    = self._pp,
                metallicity_profile = self._mp,
                cooling_fns         = cooling_fns,
            )                                                    # (Nb, Nk, NM)
            Xb = np.asarray(Xb, dtype=float) / lam_ref[:, None, None]
            out.append([Xb[b] for b in range(Xb.shape[0])])
        return out
    ...
    def angular_cl_gX(
        ...
        beta_pressure: float | None = None,
        agn_kwargs: dict | None = None,
        x_uk_override: "list | np.ndarray | None" = None,
    ) -> "np.ndarray | dict":
        ...
        # Each z-point is independent. Serial by default (n_workers=1); pass
        # n_workers>1 (or -1 for all cores) to thread, after a serial warm-up
        # compile — concurrent JAX compilation from threads can otherwise segfault.
        # ------------------------------------------------------------------ #
        nz = len(z_arr)
        ...
        def _tables_at_z(i):
            return self._pk_tables_gX(
                float(z_arr[i]), theta_cosmo, hod_params,
                beta_gas=beta_gas,
                beta_pressure=beta_pressure,
                agn_kwargs=agn_kwargs,
                x_uk_override=(None if x_uk_override is None else x_uk_override[i]),
            )

        if _nw == 1 or nz == 1:
            raw_tables = [_tables_at_z(i) for i in range(nz)]
        else:
            # Serial JAX warm-up: compile the per-z tables once before threading.
            # Concurrent JAX *compilation* from multiple Python threads can segfault;
            # dispatching already-compiled executables across threads is safe.
            _tables_at_z(0)
            from concurrent.futures import ThreadPoolExecutor
            with ThreadPoolExecutor(max_workers=min(_nw, nz)) as pool:
                raw_tables = list(pool.map(_tables_at_z, range(nz)))
